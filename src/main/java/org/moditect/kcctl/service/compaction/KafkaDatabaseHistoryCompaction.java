...
public class KafkaDatabaseHistoryCompaction {
    ...
    public KafkaDatabaseHistoryCompaction(Context context, String compactedHistoryTopic) {
    ...
    public void configure(Configuration config, HistoryRecordComparator comparator, DatabaseHistoryListener listener, boolean useCatalogBeforeSchema) {
        ...
        this.consumerConfig = config.subset(CONSUMER_PREFIX, true).edit()
                .withDefault(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, context.getBootstrapServers())
        ...
        this.producerConfig = config.subset(PRODUCER_PREFIX, true).edit()
                .withDefault(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, context.getBootstrapServers())
    ...
    public final void record(Map<String, ?> source, Map<String, ?> position, Tables schema, DdlParser parser)
        ...
        compactedSchema.tableIds().forEach(tableId -> {
            ...
            HistoryRecord compactedRecord = new HistoryRecord(source, position, "", tableId.schema(), null, tableChanges);
    ...
    protected void recoverRecords(Consumer<HistoryRecord> records) {
        try (KafkaConsumer<String, String> historyConsumer = new KafkaConsumer<>(consumerConfig.asProperties())) {
            ...
            LOGGER.debug("Subscribing to database history topic '{}'", context.getOffsetTopic());
            historyConsumer.subscribe(Collect.arrayListOf(context.getOffsetTopic()));
                        ...
                        if (lastProcessedOffset < record.offset()) {
                            if (record.value() == null) {
                                LOGGER.warn("Skipping null database history record. " +
                                        "This is often not an issue, but if it happens repeatedly please check the '{}' topic.", context.getOffsetTopic());
                            }
                            else {
                                ...
                                if (recordObj == null || !recordObj.isValid()) {
                                    LOGGER.warn("Skipping invalid database history record '{}'. " +
                                            ...
                                            recordObj, context.getOffsetTopic());
                                }
                                else {
    ...
    private Long getEndOffsetOfDbHistoryTopic(Long previousEndOffset, KafkaConsumer<String, String> historyConsumer) {
        Map<TopicPartition, Long> offsets = historyConsumer.endOffsets(Collections.singleton(new TopicPartition(context.getOffsetTopic(), PARTITION)));
